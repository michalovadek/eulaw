...
in_force <- elx_make_query("any", sector = 3,
  ...
  elx_run_query() |> 
  select(-work) |> 
  rename(date_force = `callret-3`) |> 
  arrange(celex, date_force) |> 
  distinct(celex, .keep_all = TRUE) |> 
  drop_na() |> 
  filter(between(as.Date(date_force), as.Date("1952-01-01"), as.Date(Sys.Date())))
...
The most common legal basis historically is `r most_common_lbs$title[1]`. It has been used as a legal basis on ```r most_common_lbs$n[1]``` occasions `r if(str_detect(most_common_lbs$lbcelex[1],"^3")){"and it is a non-treaty legal basis"}`. `r if(str_detect(most_common_lbs$title[1],"Merger")){"The prevalence of this particular piece of legislation underscores the EU's regulatory role in trying to ensure a competitive playing field in the single market."}` `r if(most_common_lbs$lbcelex[1] %in% in_force$celex){"It is still in force."} else {"It is no longer in force."}`
...
Excluding votes where all governments voted in favour, we are left with between ```r max(country_votes_prop$n_votes, na.rm = T)``` and ```r min(country_votes_prop$n_votes, na.rm = T)``` per Member State. While these numbers do not represent the entire historical voting record, they should still help us lift the veil on variation in Member States' propensity to disagree. Note that due to opt-outs not all countries have participated in every vote.
...
```{r votingdata, eval=FALSE, echo=TRUE}
```

## Code: Plot

```{r votesproportion, eval=FALSE, echo=TRUE}
```
...
A more sophisticated way of scaling Member States' preferences would involve deriving their ideal points from the votes through an item-response model. In our example, we assume that all votes are equally informative and important; ideally, we would want to relax this assumption.

# Todolist

In the future this page should include information on: most common Treaty legal bases (after applying equivalence table), relatedly distribution of directory codes, legal basis Treaty chapters, analysis of effect of voting rule change (RDD/DiD), and more.
