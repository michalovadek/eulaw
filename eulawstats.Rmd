...
recent_titles |> 
  datatable(rownames = FALSE,
            options = list(pageLength = 5,
                           autoWidth = TRUE,
                           columnDefs = list(list(targets = 1, width = '60px')))) |> 
...
A more sophisticated way of scaling Member States' preferences would involve deriving their ideal points from the votes through an item-response model.^[An example application of an ideal point IRT model can be found in [this paper](https://journals.sagepub.com/doi/full/10.1177/1465116520967414).] In our example, we assume that all votes with a disagreement are equally informative and important; ideally, we would want to relax this assumption.

<!-- # Todolist -->

<!-- In the future this page should include information on: distribution of directory codes (/EuroVoc terms), legal basis Treaty chapters, analysis of effect of voting rule change, and more. -->
