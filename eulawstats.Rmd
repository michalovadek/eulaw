...
nyeardif <- ifelse(n_year1 < n_year2, "less than", "more")
nyeardif <- ifelse(n_year1 == n_year2, "the same as", nyeardif)

```

In ```r year1```, the EU produced ```r n_year1``` legal acts which is ```r nyeardif``` than the ```r n_year2``` acts in ```r year2```. Overall, the EU's legislative output is currently considerably lower compared to, in particular, the 1980s and 1990s.
...
iplot_outliers <- outliers_top_txt |> 
  ...
  geom_point_interactive(aes(tooltip = title, data_id = celex), show.legend = FALSE) +
  theme_minimal() +
  scale_fill_brewer(palette = "Spectral") +
  scale_color_brewer(palette = "Spectral") +
  theme_minimal() +
